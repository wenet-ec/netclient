...
func (b *MagicBind) sendHybrid(buff []byte, ep *hybridEndpoint) error {
	...
	if !ep.peerKey.IsZero() {
		if err := b.sendDERP(buff, ep.peerKey); err != nil {
			slog.Debug("MagicBind: hybrid DERP send failed", "peer", ep.peerKey.ShortString(), "error", err)
		}
	}
...
//  2. Fallback when the live fetch fails after the TTL has expired — always
//     more accurate than the hardcoded map which rots over time.
var (
...
//  2. Tailscale's public live map (20+ regions, always up-to-date)
//  3. DERP_MAP_URLS env var — comma-separated admin-served maps merged on top
//  4. Last known good map — cached in memory from the most recent successful load
...
// A region that disappears from the live map is evicted on the next refresh.
func LoadDERPMap() *tailcfg.DERPMap {
	// TTL fast path — return cached map if it's fresh enough
	...
	// Cache the fully merged map with timestamp — TTL fast path keys off this
